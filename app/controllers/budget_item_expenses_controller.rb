class BudgetItemExpensesController < ApplicationController
  ...
  def create
    ...
    if @bi.save
      respond_to do |format|
        format.html { redirect_to root_path(budget_id: @bi.budget_item.budget_category.budget.id, was_budget: 'true', c_id: @bi.budget_item.id); flash[:notice] = 'Created Budget Item Expense' }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { flash[:notice] = "Deleted Budget Item Expense"; redirect_to root_path(budget_id: b_id, was_budget: 'true', c_id: c_id) }
