...
export default class AllocationPlans extends React.Component {
  ...
  menu() {
    return (
      <Menu>
        <Menu.Item>
          <a className="primary-color" onClick={this.editPlan}>
            <Icon type="edit" /> Edit
          </a>
        </Menu.Item>
        <Menu.Item>
          <a className="alert-color" onClick={this.deletePlan}>
  ...
  _fetchDataDone = data => {
    ...
    this.setState({
      budget: budget,
      currentPlan: budget.allocation_plans.length
        ? budget.allocation_plans[0].allocation_plan.id.toString()
        : undefined,
      allocation_plan: budget.allocation_plans.length
        ? budget.allocation_plans[0].allocation_plan
        : undefined,
    });
  ...
  _fetchBudget = data => {
    allPlans(data).then(this._fetchDataDone).catch(this._fetchDataFail);
  };
  ...
  updateBudgetItem = (index, group, updatedItem, newAmount) => {
    ...
    allocation_plan.item_groups[group_index].budget_items[
      index
    ].amount_budgeted = newAmount;
    const idx = _.findIndex(this.state.budget.allocation_plans, p => {
      return p.allocation_plan.id === allocation_plan.id;
    });
  ...
  _updateBudgetItem = (budgetItem, updatedItem) => {
    let allocation_plan = _.merge({}, this.state.allocation_plan);
    let groupIndex = _.findIndex(allocation_plan.item_groups, {
      category_id: budgetItem.budget_category_id,
    });
    let itemIndex = _.findIndex(
      allocation_plan.item_groups[groupIndex].budget_items,
      { budget_item: { id: budgetItem.id } }
    );
    let originalItem =
      allocation_plan.item_groups[groupIndex].budget_items[itemIndex];
  ...
  _saveItemFail = (originalItem, errors) => {
    let allocation_plan = _.merge({}, this.state.allocation_plan);
    let groupIndex = _.findIndex(allocation_plan.item_groups, {
      category_id: originalItem.budget_category_id,
    });
    let itemIndex = _.findIndex(
      allocation_plan.item_groups[groupIndex].budget_items,
      { budget_item: { id: originalItem.id } }
    );
  ...
  addPlan = () => {
    this.setState({
      showPlanForm: true,
      modalPlan: { income: 0, start_date: today(), end_date: today() },
    });
  };
  ...
  planSaved = plan => {
    ...
    let idx = _.findIndex(budget.allocation_plans, p => {
      return p.allocation_plan.id === plan.id;
    });
    ...
    this.setState({
      budget,
      allocation_plan,
      currentPlan: allocation_plan.id.toString(),
    });
  ...
  deletePlan = async () => {
    try {
      ...
      if (resp.success) {
        const planIdx = _.findIndex(this.state.budget.allocation_plans, {
          id: id,
        });
        const allocation_plans = _.merge(
          {},
          {},
          this.state.budget.allocation_plans.splice(planIdx, 1)
        );
  ...
  handleChange = currentPlan => {
    const allocation_plan = _.find(this.state.budget.allocation_plans, p => {
      return p.allocation_plan.id === parseInt(currentPlan);
    }).allocation_plan;
        ...
        <Col span={20}>
          <Row>
                  ...
                  <Tabs
                    ...
                    tabBarExtraContent={
                      <Button type="primary" onClick={this.addPlan}>
                        <Icon type="plus-circle" />New Pay Period
                      </Button>
                    }
                  >
                        ...
                        <TabPane tab={plan.tab_date} key={plan.id}>
                          {plan.item_groups.map((group, index) => {
                            const data = group.budget_items.map((i, k) => {
                              const total =
                                parseFloat(i.other_allocated) +
                                parseFloat(i.amount_budgeted);
                              const remaining = _.round(
                                parseFloat(i.budget_item.amount) - total,
                                2
                              );
                              return {
                                key: String(k),
                                ...
                                budgeted: (
                                  <InputNumber
                                    onChange={this.updateBudgetItem.bind(
                                      this,
                                      k,
                                      group.name,
                                      i
                                    )}
                                    value={i.amount_budgeted}
                                  />
                                ),
                                ...
                                action: (
                                  <Button
                                    type="primary"
                                    onClick={this.saveBudgetItem.bind(this, i)}
                                  >
                                    Save
                                  </Button>
                                ),
                            ...
                            return (
                              ...
                              >
                                <Card noHovering title={group.name}>
                                  <Table
                                    pagination={false}
                                    bordered
                                    dataSource={data}
            ...
            <Col span={8}>
              ...
              >
                <AllocationPlanForm
                  plan={this.state.modalPlan}
                  save={this.savePlan}
                  update={this.updateModalPlan}
                />
              </Modal>
            </Col>
          </Row>
