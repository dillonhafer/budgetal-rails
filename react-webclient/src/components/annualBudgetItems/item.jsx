...
export default class AnnualBudgetItem extends React.Component {
  ...
  getProgressModal(item, visible) {
    const startDate = moment(item.due_date).subtract(
      item.payment_intervals + 1,
      'months'
    );
  ...
  getMenu() {
    return (
      <Menu>
        <Menu.Item>
          <a className="primary-color" onClick={this.showProgress}>
            <Icon type="area-chart" /> Progress
          </a>
        </Menu.Item>
        <Menu.Item>
          <a className="primary-color" onClick={this.editItem}>
            <Icon type="edit" /> Edit
          </a>
        </Menu.Item>
        <Menu.Item>
          <a className="alert-color" onClick={this.deleteItem}>
